...
pacman::p_load(tidyverse, magrittr, stringr, scales, lubridate, tidytext, tidyr, ggthemes, janitor, Rfacebook, plotly, SocialMediaLab, geojsonio, geojsonlint, highcharter, xts, knitr, kableExtra, formattable)
...
Report {data-icon="fa-user-circle-o"}
=========================================
...
Likes, Follows and Reach by Gender {data-icon="fa-lightbulb-o" data-navmenu="Audience Age and Gender"}
=========================================
...
ggplot(aes(x=Age, y=Likes, fill=Gender)) + 
  ...
  ggtitle("Total Page Likes by Gender and Age (XXX on January 7th 2018)") +
  xlab("Age Groups") +
  ylab("Likes") + 
  guides(fill=guide_legend(title="")) + coord_flip()

  plotly::ggplotly(gg_plot, height = 760)  %>%
...
ggplot(aes(x=Age, y=Follows, fill=Gender)) + 
  ...
  ggthemes::theme_hc() +
  ggthemes::scale_fill_fivethirtyeight() +
  ggtitle("Total Page Follows by Gender and Age (XXX on January 7th 2018)") +
  xlab("Age Groups") +
  ylab("Follows") + 
  guides(fill=guide_legend(title="")) + coord_flip()
...
ggplot(aes(x=Age, y=Reach, fill=Gender)) + 
  ...
  ggthemes::theme_hc() +
  ggthemes::scale_fill_fivethirtyeight() +
  ggtitle("Page Reach by Gender and Age (XXX on January 7th 2018)") +
  xlab("Age Groups") +
  ylab("Unique Users") + 
  guides(fill=guide_legend(title="")) + coord_flip()
...
#  ) %>% 
  ...
  name = "Total Follow", type = "area", color = "green"
  ) %>% 
#  hc_add_series(
...
colnames(reaching) <- c("Measure", "Total Number (October 9th 2017 - January 7th 2018)")
...
Consumers and Consumption on Facebook Page  {data-navmenu="Page Impact"}
=========================================
...
highchart(type = "stock")  %>%
  ...
  name = "Daily Page Consumptions", type = "area", color = "red"
  ) %>% 
  hc_add_series(
...
Engagement and Engaged Users on Facebook Page  {data-navmenu="Page Impact"}
=========================================
...
colnames(viewing) <- c("Measure", "Total Number (October 9th 2017 - January 7th 2018)")
...
#  summarize(follows = sum(follows, na.rm = T)) %>% 
  ...
  highcharter::hc_title(
    text = paste0("Total Page Likes (",total, " on January 7th 2018)")
  ) %>% 
...
#  summarize(follows = sum(follows, na.rm = T)) %>% 
  ...
  highcharter::hc_colorAxis(stops = stops) %>%
  highcharter::hc_title(
    text = paste0("Total Page Follows (",total, " on January 7th 2018)")
  ) %>% 
  highcharter::hc_subtitle(text = "Based on Data from Omar Saif Ghobash's Facebook page.") %>%
#  highcharter::hc_credits(enabled = TRUE, text = "Based on Data from Omar Saif Ghobash's Facebook page") %>% 
  highcharter::hc_add_theme(highcharter::hc_theme_smpl()) %>% 
...
city_daily_reach %>% 
  ...
  highcharter::hc_subtitle(text = "Based on Data from Omar Saif Ghobash's Facebook page. By user country in unique users (N = 185.502)") %>%
#  highcharter::hc_credits(enabled = TRUE, text = "Based on Data from Omar Saif Ghobash's Facebook page") %>% 
  highcharter::hc_add_theme(highcharter::hc_theme_smpl()) %>% 
  highcharter::hc_mapNavigation(enabled = TRUE)  %>% 
